...
<header class="flex items-center justify-between px-20 py-5">
    ...
    <h2 class="text-6xl font-extrabold mb-9 text-center">CITAS MEDICAS</h2>
...
<th class="px-6 py-4 text-left text-base font-medium text-white">Acción</th>
        ...
        $total_consulta = $conexion->query("
            ...
            INNER JOIN PERSONA P ON PACIENTE.PERSONA_ID = P.ID
            LEFT JOIN TELEFONO ON P.ID = TELEFONO.Persona_ID
            INNER JOIN EMPLEADO ON CITA_MEDICA.DOCTOR_ID = EMPLEADO.ID
            INNER JOIN PERSONA PD ON EMPLEADO.Persona_ID = PD.ID
            LEFT JOIN EMPLEADO_has_ESPECIALIDAD EHE ON EMPLEADO.ID = EHE.EMPLEADO_ID
            LEFT JOIN ESPECIALIDAD E ON EHE.ESPECIALIDAD_ID = E.ID
            WHERE 
                CONCAT(P.PNombre, ' ', P.SNombre, ' ', P.PApellido, ' ', P.SApellido) LIKE '%$busqueda%'
                OR CONCAT(PD.PNombre, ' ', PD.PApellido) LIKE '%$busqueda%'
        ...
        if ($consulta->num_rows > 0) {
            while ($row = $consulta->fetch_assoc()) {
                echo "<tr class='border-t bg-[#f9fafb]'>
                    <td class='px-6 py-4'>{$row['CitaID']}</td>
                    <td class='px-6 py-4'>{$row['Fecha_Hora']}</td>
                    <td class='px-6 py-4'>{$row['PacienteNombreCompleto']}</td>
                    <td class='px-6 py-4'>{$row['TelefonoPaciente']}</td>
                    <td class='px-6 py-4'>{$row['DoctorNombreCompleto']} {$row['Especialidad']}</td>
                    <td class='px-6 py-4'>{$row['Observaciones']}</td>
                    <td class='px-6 py-4'>{$row['ESTADO']}</td>
                    <td class='px-6 py-4'>{$row['HabitacionNumero']}</td>
                    <td class='px-6 py-4'>
                       <a href='AgregarConsulta.php?id={$row['CitaID']}' 
            ...
            echo "<tr class='bg-[#f9fafb]'><td colspan='9' class='px-6 py-4 text-center text-gray-500'>No se encontraron resultados</td></tr>";
        }
        ?>
        </tbody>
    </table>
